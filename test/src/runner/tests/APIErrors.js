...
export function run({ Module, data, env, chai })
{
   ...
   describe(`API Errors (${data.scopedName}):`, () =>
   {
      describe('connect:', () =>
      {
         ...
         {
            const socket = new WSClient({ port: 8001 });
      ...
      describe('setClientOptions:', () =>
      {
         ...
         {
            expect(() => new WSClient({ port: 0,  binaryType: 'bad' })).to.throw(TypeError,
             `'clientOptions.binaryType' must be 'blob' or 'arraybuffer'.`);
         });

         it(`'clientOptions.serializer' does not conform to the JSON API. (not object)`, () =>
         {
            expect(() => new WSClient({ port: 0,  serializer: 'bad' })).to.throw(TypeError,
      ...
      if (env.isNode)
      {
         describe('wsOptions', () =>
         {
            ...
            {
               expect(() => new WSClient({ port: 8001 }, false)).to.throw(TypeError, `'wsOptions' is not an object.`);
            });
